...
class HrExpenseSheet(models.Model):
    ...
    # CALCULO DEL TOTAL DE GASTOS VERIFICADOS
    @api.depends('expense_line_ids.real_expenses', 'expense_line_ids.verified')
    def _compute_total_expenses_verified(self):
        for sheet in self:
            verified_lines = sheet.expense_line_ids.filtered(lambda line: line.verified)
            sheet.total_expenses_verified = sum(verified_lines.mapped('real_expenses'))


        # CALCULO DE DIFERENCIA ENTRE DOS CAMPOS
    @api.depends('total_expenses_verified', 'total_amount')
    def _compute_diferenc(self):
        for record in self:
            total_expenses_verified = record.total_expenses_verified or 0.0
            total_amount = record.total_amount or 0.0
            record.refund = float(total_amount - total_expenses_verified)
    ...
    # ACCION DEL BOTON SETTLE ADVANCE
    ...
    def _prepare_bills_vals(self):
        ...
        if self.is_liquidation and self.refund > 0:
